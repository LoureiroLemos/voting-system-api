...
class Controller {
    async create(req, res) {
        ...
        try {
            ...
            res.status(201).json({
                message: "Enquete criada com sucesso!", pollId
            });
        } catch (error) {
            console.error(error);
            res.status(500).json({
    ...
    async findOne(req, res){
        ...
        try {
            ...
            const queryOptions = "SELECT id, option_text, votes FROM poll_option WHERE poll_id = ?";
    ...
    async vote(req, res) {
        ...
        try {
            const query = "UPDATE poll_opttions SET votes = votes + 1 WHERE id = ?";
